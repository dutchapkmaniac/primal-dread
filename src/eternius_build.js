...
export function buildCity(city) {
  ...
  const sandLit = (rx, rz) => { const m = sand(rx, rz); if (m.map) { m.emissiveMap = m.map; m.emissive = new THREE.Color(0xffffff); m.emissiveIntensity = 0.36; } else m.emissiveIntensity = 0.5; return m; };   // update 48: down to the floor's tone (1.5x too bright next to it)   // update 44/47: the big walls you face — lit by their OWN texture, so they read as sandstone with no lamp near (a flat emissive was a dark brown 'void')
  ...
  city.flames = []; city.lanterns = []; city.lights = []; city.flags = []; city.banners = []; city.walls = []; city.doors = []; city.nightGlows = []; city.keepExtra = []; city.tunnelWater = []; city.swing = [];
  // update 45: the flags' cloth is DARK GREEN, not black — the emblem texture's dark background is recoloured once (the
  // emblem itself untouched); every flag, banner and pennant draws from this copy
  city.flagTex = A.tex.t_flag || null;
  if (A.tex.t_flag && A.tex.t_flag.image) {
    try {
      ...
      for (let i = 0; i < d.length; i += 4) { const lum = (d[i] * 0.3 + d[i + 1] * 0.59 + d[i + 2] * 0.11) / 255; if (lum < 0.3) { const k = 1 - lum / 0.3; d[i] = d[i] * (1 - k) + 30 * k; d[i + 1] = d[i + 1] * (1 - k) + 88 * k; d[i + 2] = d[i + 2] * (1 - k) + 46 * k; } }
  ...
  }   // update 44: keepExtra = animated meshes the merger must leave alone
  ...
  city.prims = { box, sector, cyl, wallSeg, obst, pillar, archFrame, archFill, archWall, prop, archShape };
  ...
  city.addFlag = (bx, y, az, faceDeg, tall = false) => {
    ...
    const m = new THREE.MeshStandardMaterial({ map: tex || null, color: tex ? 0xffffff : 0x1f5a2a, side: THREE.DoubleSide, roughness: 0.9, emissive: 0xffffff, emissiveMap: tex || null, emissiveIntensity: 0.45 });
  ...
  city.addBanner = (bx, y, az, faceDeg, wd = 2.4, h = 9) => {
    const tex = city.flagTex || A.tex.t_flag;
    const m = new THREE.MeshStandardMaterial({ map: tex || null, color: tex ? 0xffffff : 0x1f5a2a, side: THREE.DoubleSide, roughness: 0.9, emissive: 0xffffff, emissiveMap: tex || null, emissiveIntensity: 0.45 });
    if (m.map) { m.map = m.map.clone(); m.map.repeat.set(1, h / (wd * 1.33)); m.map.wrapT = THREE.RepeatWrapping; m.map.needsUpdate = true; m.emissiveMap = m.map; }
...
function buildMountain(city, grp, mountainM, dark, rock) {
  ...
  for (let i = 0; i <= NR + 1; i++) {
    for (let j = 0; j <= NT; j++) {
      ...
      // update 45: the foot ring stays on the ground (a lifted foot put the mountain's skin INSIDE the court beside the
      // gate); the ring behind it rises over the corridor, and the faces of the foot band are cut where the corridor
      // passes (below), so the rock is only ever behind the castle's back wall
      if (lz > 0 && Math.abs(lx) < C.tunnel.hw + 2.4 && i === NR - 1) h = Math.max(h, C.levels.court + 19.5);
  ...
  for (let i = 0; i < NR + 1; i++) for (let j = 0; j < NT; j++) {
    if (i === NR || i === NR - 1) { const th = ((j + 0.5) / NT) * Math.PI * 2; if (Math.cos(th) > 0 && Math.abs(Math.sin(th)) * R < C.tunnel.hw + 3.2) continue; }   // update 42/45: no apron and no foot band across the corridor
...
function buildCastle(city, grp, box, sand, rock, gold, goldPlain, goldBright, gem, dark, wallSeg, obst, archFrame, archFill, archWall, latticeM, prop, waterMat, pillar) {
  ...
  const wallRun = (bx0, bx1, az, sideWall = false) => {
    ...
    for (let i = 0; i < m; i += 2) { const p = (!sideWall && bx1 <= 0) ? bx1 - 1.2 - i * 2.4 : bx0 + 1.2 + i * 2.4; if (sideWall) box(T + 0.2, 1.1, 1.4, az, y0 + H + 1.25, p, sand(1, 1)); else box(1.4, 1.1, T + 0.2, p, y0 + H + 1.25, az, sand(1, 1)); }
  };
  ...
  for (const az of [K.a1, K.a0]) {
    ...
    for (const s of [-1, 1]) {
      ...
      box(8.6, 1.0, 8.6, tb, y0 + 0.5, az, az === K.a1 ? gold : sand(2, 1));   // update 45: gold on the bridge side, sandstone inside — nothing black
  ...
  // it to the dome's foot, gold cornices top and bottom; the rock's notch is behind all of it
  {
    ...
    box(gw + 0.6, 0.6, gd + 0.6, 0, yt + 0.3, ga, gold); box(gw + 0.6, 1.0, gd + 0.6, 0, yb - 0.5, ga, gold);   // update 45: the lower cornice reaches down to the hallway's roof
  ...
  for (const s of [-1, 1]) city.addTorchbearer(K.a0 + 7.0, s * (C.tunnel.hw + 6), y0, 90 * (s > 0 ? -1 : 1) + 0);   // update 46: in front of the towers, not inside them
  ...
  for (const s of [-1, 1]) {
    ...
    // update 45: clear of the palms and pillars (6 m in from the palm line), turned to face the statue in the middle
    if (benchA) for (const az of [K.a0 + 10, ac, K.a1 - 10]) { const bch = city.warmProp(benchA.model.clone()); const [x, z] = cityWorld(az, s * (K.hw - 14)); bch.position.set(x, y0, z); bch.rotation.y = C.grpYaw + (s > 0 ? Math.PI : 0); city.g.scene.add(bch); obst(az, s * (K.hw - 14), 1.4); }   // the seat toward the statue, the back to the colonnade
  }
...
function buildCavern(city, grp, box, sector, cyl, sand, rock, gold, goldPlain, goldBright, gem, dark, wallSeg, obst, pillar, archFrame, archWall, carpetM, waterMat, prop, glowM, greenGlowM, latticeM) {
  ...
  const stone = sand(4, 4); if (stone.map) { stone.emissiveMap = stone.map; stone.emissive = new THREE.Color(0xffffff); stone.emissiveIntensity = 0.14; }   // update 48: the treads match the floor   // update 47: the treads and their sides keep their texture in the dark too

  // ---- floors ----
  {   // update 43: the plaza floor with the down stair's trench cut out of it (the slab used to run over the treads)
    // update 46: the outline itself is notched for the trench (a hole had to stay inside the disc, which left a half-metre
  ...
  { const g = (SD_hw) => (SD_hw / TR) / D2R; const dth = g(C.stairs.down.hw + 0.4); for (const mm of [cyl(TR, S.stairTh1, -90 - dth, L.lower - 1, L.plaza, city.mats.sandLit((-90 - dth - S.stairTh1) * 0.4, 2.75)), cyl(TR, -90 + dth, -ET, L.lower - 1, L.plaza, city.mats.sandLit((-ET + 90 - dth) * 0.4, 2.75))]) mm.material.side = THREE.DoubleSide; }   // update 45/46: both faces, one texture scale   // update 43: a gap for the stair's trench, blocks not smears
  ...
  // the river's water, its culvert arches (the water flows in from the dark and out again), golden grates
  {
    ...
    // update 45: the culvert — the end wall flush against its wall, its pointed arch springing from the water; behind it
    ...
    city.culvert = (thd, sgn, yTop, grade = 0) => {   // grade: +1 the water comes DOWN to you, -1 it runs away downhill
      ...
      const segL = 5, nSeg = Math.round(CV.depth / segL), slope = grade * Math.atan2(1.0, 30), dth = segL / rm;
      const tunM = sand(2, 3); tunM.side = THREE.BackSide;
      const tw = waterMat(); tw.map && tw.map.repeat.set(2, 3);   // update 46: the river's own water, the same blue inside and out, day and night
      const segGeo = (() => {   // the extruded arch WITHOUT its two flat end caps (they stood across the tube every 5 m)
        const ex = new THREE.ExtrudeGeometry(city.prims.archShape(W, Hh + 0.2, 0, true), { depth: segL + 0.3, bevelEnabled: false }); ex.translate(0, -0.1, -(segL + 0.3) / 2);
        const gs = ex.groups.find((g) => g.materialIndex === 1) || { start: 0, count: ex.attributes.position.count };
        const pick = (at) => new THREE.BufferAttribute(at.array.slice(gs.start * at.itemSize, (gs.start + gs.count) * at.itemSize), at.itemSize);
        const g2 = new THREE.BufferGeometry(); g2.setAttribute("position", pick(ex.attributes.position)); g2.setAttribute("uv", pick(ex.attributes.uv));
        const uv = g2.attributes.uv, pp = g2.attributes.position; for (let i = 0; i < uv.count; i++) uv.setXY(i, (pp.getX(i) + pp.getZ(i)) / 3, pp.getY(i) / 3);
        g2.computeVertexNormals(); g2.computeBoundingSphere(); return g2;
      })();
  ...
  for (const sgn of [1, -1]) {
    ...
    for (let i = 1; i <= n; i++) {
      ...
      const h = top - bottom;
      box(run + 0.02, h, ST.hw * 2, sgn * (b0 + run / 2), bottom + h / 2, 0, (sgn > 0 && i % 5 === 0) ? gold : stone);   // update 45: the river stair is plain stone
      if (sgn > 0) box(0.12, 0.05, ST.hw * 2, sgn * (b0 + 0.06), top + 0.02, 0, goldPlain);   // update 45: no gold strips on the river stair
    }
    ...
    if (sgn > 0) { const bEnd = Math.min(ST.r1, TR - 0.4), slope = Math.atan2(y1 - L.plaza, ST.r1 - ST.r0), len = (bEnd - ST.r0) / Math.cos(slope), bm = (ST.r0 + bEnd) / 2, ym = L.plaza + (y1 - L.plaza) * (bm - ST.r0) / (ST.r1 - ST.r0) - 0.75;
      for (const s of [-1, 1]) { const sk = box(len, 1.4, 0.3, bm, ym, s * (ST.hw + 0.2), city.mats.sandLit(Math.max(1, Math.round(len / 4)), 1)); sk.rotation.z = slope; } }   // update 48: a sandstone skirt under the balustrade, the whole length, both sides
  }
  // ---- the grand stair: from the terrace (+8) down the outer band to the gallery (-14), a landing halfway ----
  {
    ...
    // update 46: the inner wall is seen from the plaza as well (both faces, lit) — and the part above the plaza is ONE
    // stepped strip with the very same texture scale, not a face per step (each restarted the pattern and looked squeezed)
    { const wm = city.mats.sandLit((th1 - th0 + 0.6) * 0.4, 2.75); wm.side = THREE.DoubleSide; cyl(TR, th0 - 0.3, th1 + 0.3, L.lower - 1, L.plaza + 0.05, wm, false, 24); }
    cyl(TR + 0.3, th0 - 0.3, th1 + 0.45, L.plaza - 0.55, L.plaza - 0.05, gold, false, 24); cyl(TR + 0.3, th0 - 0.3, th1 + 0.45, L.lower + 0.05, L.lower + 0.55, gold, false, 24);   // update 48: the two gold beams run on straight along this wall (gallery side) up to the retaining wall's beams
    {   // update 48: the wall you face from the plaza is SOLID — one 0.8 m sandstone box per step, its top stepping
        // up with the stair, the block texture running on from box to box (uv from the arc length and the height)
      const WT = 0.8, kU = 0.25;
      const wallBox = (tA, tB, yTop) => {
        const tm = (tA + tB) / 2, wd = (tB - tA) * D2R * TR + 0.02, yB = L.plaza - 0.5, h = yTop - yB;
        const wm = city.mats.sandLit(1, 1);
        const bx = box(wd, h, WT, (TR - WT / 2) * Math.sin(tm * D2R), yB + h / 2, (TR - WT / 2) * Math.cos(tm * D2R), wm, tm * D2R);
        const g2 = bx.geometry, pp = g2.attributes.position, uv = g2.attributes.uv, nn = g2.attributes.normal, arc = tm * D2R * TR;
        for (let k = 0; k < pp.count; k++) { const ax = Math.abs(nn.getX(k)) > 0.5 ? pp.getZ(k) : pp.getX(k); uv.setXY(k, (arc + ax) * kU, (yB + h / 2 + pp.getY(k)) * kU); }
        uv.needsUpdate = true;
      };
      for (let i = 1; i <= n; i++) {
        const tA = th0 + stepArc * (i - 1) + (i > half ? land : 0), tB = tA + stepArc, y = L.terrace - rs * i;
        if (y <= L.plaza + 0.05) continue;
        wallBox(tA, tB, y);
        if (i === half) wallBox(tB, tB + land, y);
      }
      // update 47/48: the gold bands on the plaza side — the bottom one straight along the base, the top one following
      // the steps along the wall's top edge, both standing just proud of the boxes
      cyl(TR - WT - 0.05, th0 - 0.3, th1 + 0.3, L.plaza + 0.05, L.plaza + 0.55, gold, true, 24);
      { const bp = [], bi = []; let bv = 0; const rb = TR - WT - 0.05;
        const band = (tA, tB, y) => { for (const [t, yy] of [[tA, y - 0.5], [tB, y - 0.5], [tB, y], [tA, y]]) bp.push(rb * Math.sin(t * D2R), yy, rb * Math.cos(t * D2R)); bi.push(bv, bv + 1, bv + 2, bv, bv + 2, bv + 3); bv += 4; };
        for (let i = 1; i <= n; i++) {
          const tA = th0 + stepArc * (i - 1) + (i > half ? land : 0), tB = tA + stepArc, y = L.terrace - rs * i;
          if (y <= L.plaza + 0.6) continue;
          band(tA, tB, y); if (i === half) band(tB, tB + land, y);
        }
    ...
    // update 43: the channel's walls and bed, the arch the water comes out of (lit inside), and fences on both banks of the channel
    {
      ...
      city.culvert(tc0, -1, city.grandStairY(tc0 - 0.2) + 0.4, 1);   // update 45: the gentle uphill tunnel
  ...
  // columns, a prayer table with offering flames and kneeling cushions, and chains with censers hung from the rim of the hole ----
  {
    ...
    if (altarModel && altarModel.traverse) altarModel.traverse((o) => {
      ...
      try {
        ...
        for (let i = 0; i < d.length; i += 4) { const r = d[i], gg = d[i + 1], bb = d[i + 2], lum = (r * 0.3 + gg * 0.59 + bb * 0.11) / 255; const k = Math.max(0, Math.min(1, (0.44 - lum) / 0.14)); d[i] = r * 0.1 + 255 * k; d[i + 1] = gg * 0.1 + 198 * k; d[i + 2] = bb * 0.1 + 66 * k; d[i + 3] = 255; }
        ctx.putImageData(id, 0, 0);
    ...
    for (let k = 0; k < 4; k++) { const an = k * Math.PI / 2; city.altarGlowLights.push(city.emit(grp, Math.sin(an) * (DR + 1.7), yDrum0 + (yDrum1 - yDrum0) * 0.5, Math.cos(an) * (DR + 1.7), 0xffcf60, 2.2, 18, {})); }   // update 44: emerald light off the carvings, all round
  ...
  for (const s of [-1, 1]) {
    ...
    const wallPiece = (r0, r1, yLo, yHi) => { if (r1 - r0 < 0.05 || yHi - yLo < 0.05) return; const rmid = (r0 + r1) / 2; const wm = new THREE.Mesh(new THREE.BoxGeometry(1.2, yHi - yLo, r1 - r0), city.mats.sandLit(Math.max(1, Math.round((r1 - r0) / 6)), Math.max(1, Math.round((yHi - yLo) / 2)))); wm.position.set(rmid * Math.sin(th), (yLo + yHi) / 2, rmid * Math.cos(th)); wm.rotation.y = th; grp.add(wm); };
    if (s < 0) { const CV = C.culvert, rmw = (C.riverR0 + C.riverR1) / 2, tw0 = rmw - (CV.w - 1.2) / 2 - 0.5, tw1 = rmw + (CV.w - 1.2) / 2 + 0.5; wallPiece(TR, tw0, lo, hi); wallPiece(tw1, R, lo, hi); wallPiece(tw0, tw1, lo, L.water - 0.9); wallPiece(tw0, tw1, L.water - 0.5 + CV.h + 0.4, hi); }   // update 45: the river's tunnel runs through this wall
    else wallPiece(TR, R, lo, hi);   // update 44: thicker, self-lit (it read as void at night)
    ...
    { const nx = Math.sin(th), nb = -Math.cos(th);   // update 45: outward normal (a, b) of the decorated face — the court side of +ET, the GALLERY side of -ET (the old sign put -ET's statues and lamps 16 m under the court, right where the river's tunnel now runs)
      const inRiver = (d) => s < 0 && TR + d > C.riverR0 - 1.5 && TR + d < C.riverR1 + 1.5;   // nothing standing in the river or on the culvert's arch
      const foot = s > 0 ? L.court : L.lower, top = hi, dirDeg = Math.atan2(nb, nx) / D2R;
      for (let d = 2; d < R - TR - 1; d += 4) { if (inRiver(d)) continue; const ca = a0 + (a1 - a0) * d / (R - TR), cb = b0 + (b1 - b0) * d / (R - TR); box(0.5, top - foot - 0.2, 1.0, cb + nb * 0.5, (foot + top) / 2, ca + nx * 0.5, gold, th); const gm = new THREE.Mesh(new THREE.OctahedronGeometry(0.28), gem); gm.position.set(cb + nb * 0.85, top - 1.4, ca + nx * 0.85); grp.add(gm); }
      // (update 48: the mid-height beam is gone — it lay in the wall's face and flickered)
      for (let d = 5; d < R - TR - 2; d += 8) { if (inRiver(d)) continue; const ca = a0 + (a1 - a0) * d / (R - TR), cb = b0 + (b1 - b0) * d / (R - TR); city.addPoleFlag(cb + nb * 0.6, top - 1.6, ca + nx * 0.6, dirDeg, 0.95, 4.5); city.addTorchbearer(ca + nx * 4.4, cb + nb * 4.4, foot, dirDeg); }   // update 44: the statues stand clear of the pennants
      for (let d = 9; d < R - TR - 2; d += 8) { if (inRiver(d)) continue; const ca = a0 + (a1 - a0) * d / (R - TR), cb = b0 + (b1 - b0) * d / (R - TR); city.addLamppost(ca + nx * 3.0, cb + nb * 3.0, foot, true); }   // update 44: lamps along its foot
    }
  }
  ...
  // ---- the THRONE HALL ----
  {
    ...
    for (let i = 0; i < 4; i++) for (const s of [-1, 1]) { const pa = T.a0 + 5 + i * 8; pillar(s * (hw - 3), y, pa, 0.7, H, gold); city.addPoleFlag(s * (hw - 3 - 0.72), y + 8.5, pa, s > 0 ? -90 : 90, 0.95, 5.0); }   // update 45: socketed in each pillar's inner face, leaning over the carpet
  ...
  // you can walk into it, and it is what stops you from jumping down
  {
    ...
    { let tF = S.stairTh1 - 0.3; for (let t = S.stairTh0 + 0.4; t < S.stairTh1; t += 0.1) if (city.grandStairY(t) < L.plaza + 0.5) { tF = t; break; }   // update 45: a fence only where the steps stand above the plaza (a real drop); below that the wall is the rail
      city.stairFence(TR + 0.45, S.stairTh0 + 0.4, tF, (t) => city.grandStairY(t), 1); }
  }
  ...
  // in pointed niches on corbels, five hanging chain lanterns of green glass that sway, more pennants and torch brackets
  {
    ...
    // (update 46: the three gold bands are gone — only the reliefs, colossi, lanterns, flags and lamps)
...
function buildHomes(city, grp, box, cyl, sand, rock, gold, goldPlain, goldBright, gem, dark, wallSeg, obst, archFrame, carpetM, prop, glowM, greenGlowM) {
  ...
  for (const rm of C.rooms) {
    ...
    box(RM.hw * 2 + 1, 0.4, RM.depth + 1.5, cb, y + RM.h + 0.2, ca, sand(3, 3), ry);   // update 46: sandstone like the rest of the front
...
function buildStall(city, st, grp, box, sand, gold, goldPlain, goldBright, gem, dark, obst, prop, latticeM, glowM) {
  ...
  if (st.id === "food") {
    ...
    put("axe3d", 0.2, -1.9, 1.3, Math.PI / 2, 1.1); put("knife3d", 0.1, -0.6, 1.3, Math.PI / 2, 0.35); put("torch3d", 0.1, 0.5, 1.3, Math.PI / 2, 1.1); put("crossbow3d", -2.3, 1.0, 2.75, 0, 0.9);
